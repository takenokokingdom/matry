...
モバイル・Webともに以下の構成を基本とする。

```
src/
├── features/          # 機能単位のまとまり
│   └── {feature}/
│       ├── api/       # API呼び出し（動詞-名詞.ts）
│       ├── components/ # UIコンポーネント
│       ├── hooks/     # カスタムフック
│       └── {Feature}Screen.tsx  # 画面コンポーネント
└── shared/
    └── lib/           # 目的別の共通ユーティリティ
        └── {目的}/
            └── {役割}.ts
...
**ルール**
- `{Feature}Screen.tsx` には画面コンポーネントのみ配置し、他のコンポーネントは切り出す
- featureは機能単位で分割する（例: `chat`・`preview` は別feature）
- `shared/lib` はドメイン非依存の共通関数を目的別に格納する（ライブラリ依存に限らない）

**ファイル命名規則**
- Reactコンポーネント: パスカルケース（`GeneratingIndicator.tsx`）
- それ以外: ケバブケース（`build-preview.ts`、`use-chat.ts`）
...
> 仕様承認前にブランチを切らない。手戻りを防ぐためにステップ3が完了してから実装に入る。
